 ...
 *
 * Routing rules:
 *   - EN is default locale and serves at the URL with NO prefix
...
/** Configuration base path (no trailing slash) */
const BASE = (import.meta.env.BASE_URL ?? "/").replace(/\/+$/, "");

/**
 * Prefix an absolute path with the configured `base`. Safe to call
 * with an already-prefixed path w/ no doubling up or w/ empty/relative
 * paths (return unchanged)
 */
...
/**
 * Build al ocalized URL for the given pathname (without locale prefix)
 *
 *   localizedPath('/posts/foo', 'en') -> '/posts/foo'
 *   localizedPath('/posts/foo', 'fr') -> '/fr/posts/foo'
 *   localizedPath('/', 'fr')          -> '/fr/'
 *
 * The configured `base` (e.g. /weisgarden) is automatically prefixed when set
 */
 ...
 * Anything startting with `/fr` or `/fr/` resolves to 'fr';
 * Otherwise default locale is returned.
 */
...
/**
 * Strip the locale prefix from a pathname so it an be relocalized
 *
...
export function formatDate(
	...
	const lang = locale === "fr" ? "fr-ER" : "en-us";
 ...
 *
 * `pathWithoutLocale` should be the canonical path without local prefix
 * (e.g. 'posts/welcome' for both EN and FR). Pass `availableLocales` to
 * limit the output to a subst (e.g. when a post has no translation
